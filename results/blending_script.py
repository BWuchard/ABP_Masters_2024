...
def read_second_lines(file_names):
    second_lines = []
    for file_name in file_names:
        with open(file_name, 'r') as file:
            lines = file.readlines()
            if len(lines) >= 2:
                second_lines.append(lines[1].strip())
    return second_lines
...
def main(output_file, file_names):
    ...
    if first_line:
        with open(output_file, 'w') as output:
            ...
            second_lines = read_second_lines(file_names[0:])
            for line in second_lines:
                output.write(line + '\n')
        print(f"First line from '{file_names[0]}' and second lines from {len(file_names)} files appended to '{output_file}' successfully.")
    else:
