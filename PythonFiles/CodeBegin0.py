...
antigen_doses = {'BCG':1,'DT':3,'DTP':3,'HepB':3,'Hib':3,'HPV':2,'IPV':3,'MMR':2,'OPV':4,'PCV':3,'Rota':2,'Varicella':2,'Td':3,'YF':1}
print(antigens)
n_antigens = len(antigens)
print(n_antigens)
vaccines = ['BCG', 'DT', 'DTP', 'DTP-HepB-Hib-IPV', 'DTP-HepB-IPV', 'DTP-Hib', 'DTP-Hib-IPV', 'DTP-IPV', 'DTP', 'DTP-HepB-Hib', 'HepB', 'HPV', 'IPV', 'MMR', 'PCV', 'Rota', 'Td', 'Varicella', 'Td-IPV', 'YF', 'OPV'] 
...
cdc_pr_df['Vaccine '] = cdc_pr_df['Vaccine '].replace({'TT':'Td','DTaP':'DTP','DTaP-HepB-IPV':'DTP-HepB-IPV','DTaP-HepB-Hib-IPV':'DTP-HepB-Hib-IPV','DTaP-HepB-IPV':'DTP-HepB-IPV','DTaP-Hib-IPV':'DTP-Hib-IPV','DTaP-IPV':'DTP-IPV','HepA+B':'HepB','HPV2':'HPV','HPV4':'HPV','HPV9':'HPV','MMRV':'MMR','PCV13':'PCV','Tdap':'Td','bOPV':'OPV','DTwP':'DTP','DTwP-HepB-Hib':'DTP-HepB-Hib','DTwP-Hib':'DTP-Hib','PCV10':'PCV','tOPV':'OPV'})
...
paho_pr_df['Vaccine '] = paho_pr_df['Vaccine '].replace({'TT':'Td','DTaP':'DTP','DTaP-HepB-IPV':'DTP-HepB-IPV','DTaP-HepB-Hib-IPV':'DTP-HepB-Hib-IPV','DTaP-HepB-IPV':'DTP-HepB-IPV','DTaP-Hib-IPV':'DTP-Hib-IPV','DTaP-IPV':'DTP-IPV','HepA+B':'HepB','HPV2':'HPV','HPV4':'HPV','HPV9':'HPV','MMRV':'MMR','PCV13':'PCV','Tdap':'Td','bOPV':'OPV','DTwP':'DTP','DTwP-HepB-Hib':'DTP-HepB-Hib','DTwP-Hib':'DTP-Hib','PCV10':'PCV','tOPV':'OPV'})
...
unicef_pr_df['Vaccine '] = unicef_pr_df['Vaccine '].replace({'TT':'Td','DTaP':'DTP','DTaP-HepB-IPV':'DTP-HepB-IPV','DTaP-HepB-Hib-IPV':'DTP-HepB-Hib-IPV','DTaP-HepB-IPV':'DTP-HepB-IPV','DTaP-Hib-IPV':'DTP-Hib-IPV','DTaP-IPV':'DTP-IPV','HepA+B':'HepB','HPV2':'HPV','HPV4':'HPV','HPV9':'HPV','MMRV':'MMR','PCV13':'PCV','Tdap':'Td','bOPV':'OPV','DTwP':'DTP','DTwP-HepB-Hib':'DTP-HepB-Hib','DTwP-Hib':'DTP-Hib','PCV10':'PCV','tOPV':'OPV'})
...
cp_df_sorted = country_param_df[country_param_df['country_name'].isin(selected_list)]
cp_df_sorted = cp_df_sorted.rename(columns={'country_name' : 'Country', 'annual_births' : 'Annual_births', 'GNI_c' : 'GNI_pc'})
cp_df_sorted = cp_df_sorted.sort_values(by='Country')
cp_df_sorted = cp_df_sorted.reset_index(drop=True)
cp_df_sorted['Code'] = cp_df_sorted.index + 1
...
# Read the Excel file
excel_file = 'C:\\Users\\Bailey\\Downloads\\Research\\market_assignments_1.xlsx'  # Replace with the actual file path
...
def generate_country_codes(country_list):
    country_codes.clear()  # Clear the existing dictionary
    country_codes.update({country: i for i, country in enumerate(sorted(country_list), start=1)})
...
country_param_df_sum = country_param_df['annual_births'].sum()
country_status_df_sum = country_status_df['Birth Cohort '].sum()
...
averagecoverages = pd.read_excel('averagecoverages.xlsx')

# Sort the DataFrame by the 'Country' column
averagecoverages_sorted = averagecoverages.sort_values(by='Country')

# This will add the corresponding values of the 'Coverage' column to the 'Annual_births' column
cp_df_sorted = pd.merge(cp_df_sorted, averagecoverages_sorted[['Country', 'Coverage']], on='Country', how='inner')
# Now you can rename 'Coverage' column to 'Annual_births' and perform addition
cp_df_sorted['Annual_births'] += cp_df_sorted['Coverage']


# Iterate over each market and its assigned countries
for market, selected_user in markets.items():
    # Filter the original dataframe for countries in the current market
    market_data = cp_df_sorted[cp_df_sorted['Country'].isin(selected_user)]
...
def write_output_to_file(output):
    with open("market_assignments_1.dat", "a") as file:
        file.write(output + '\n')
